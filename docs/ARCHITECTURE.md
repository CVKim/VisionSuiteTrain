...
- **_BaseVscExporter**: ONNX export + manifest/model.yaml writer + **fail-fast 정합 assert**
  (`manifest.inputs.shape[2:]==[h,w]` ∧ `label_map keys==range(len(names))` ∧ `opset 일치` ∧ `NC(onnx introspect)==len(names)`).
...
- labelme circle/line/point shape 매핑, SEG 겹침 우선순위.
