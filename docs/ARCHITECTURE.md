...
| task | ONNX 출력 | 규약 |
|---|---|---|
| **DET** | `[B, 4+NC, A]` channel-first | ch0-3=cx,cy,w,h(letterbox 입력-px), ch4..=per-class **sigmoid 적용** 점수, objectness 없음, **NMS/top-k export 금지**(VSC가 conf+NMS). **A=anchor(YOLO) 또는 query(D-FINE/RF-DETR=300)** — v7/v8/D-FINE/RF-DETR 동일 컨트랙트 |
| **CLS** | `[B, NC]` (=`[B,NC,1,1]` 동일취급) | **softmax BAKED**(참조 코어 동형, forward_export) → 출력=확률[0,1]. cls_activation 은 manifest 서술 메타 |
...
| yolov7_hbb | det | **drop-in** — 실측 v7 export 도 `[1,4+NC,A]` 통일(yolov8_hbb 디코드) | 1차(등록됨) |
| dfine_hbb | det | **drop-in** — 실측 `[1,4+NC,300]`(A=쿼리), value_range[0,H] = yolov8_hbb 컨트랙트. **신규 핸들러 불요**, 학습 repo 래핑만 | 2차(어댑터) |
| rfdetr_hbb | det | **drop-in** — `[1,4+NC,300]` 동일. yolov8_hbb 핸들러로 디코드 | 2차(어댑터) |
| foundation_anomaly | anomaly | 실측 `[1,1,H,W]` heatmap + scalar threshold → VSC foundation_anomaly 핸들러 | 2차(어댑터) |
...
- 2차 학습 어댑터: D-FINE/RF-DETR(export 는 yolov8_hbb `[1,4+NC,A]` drop-in 실측 확인 →
  학습 repo 래핑만, **신규 VSC 핸들러 불요**), foundation_anomaly(`[1,1,H,W]`), yolov8_obb.
